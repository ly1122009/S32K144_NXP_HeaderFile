...
void NVIC_Systick_Enable(volatile uint32 Reload_Value)
{
...
void LPUART_Receiver_String(LPUART_Type*LPUART_x, volatile uint8 String_result[]){
	
	volatile uint32 Count_temp = 0;
	while( String_result[Count_temp] != '\0' )
	{
		 LPUART_Receiver_Char(LPUART_x,String_result[Count_temp]);
		 Count_temp++;
	}
